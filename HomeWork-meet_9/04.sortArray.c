...
void sort_arr(void *p_arr, int num, int dir, void *ftype_fp); // function declaration
...
int main()
{
    ...
    printf("\nInput direction to sort array -> Ascending order: press (1), Descending order: press (-1)  ");
    scanf("%d", &dir);
...
void sort_arr(void *p_arr, int num, int dir, void(*ftype_fp)) // void function to return sorted array
{
    sortArray(p_arr, num, dir);
    ftype_fp = sortArray;
    // void ftype_fp(p_arr, num, dir);
    printArray(p_arr, num);
}